...
| `AIRTABLE_TOKEN` | Personal Access Token. Права: `data.records:read` + `data.records:write` (+ `schema.bases:read` для автоподтягивания баз) |
| База и таблицы | подтягиваются автоматически по токену (кнопка «Подключиться»); ID можно ввести и вручную |
| Дневная норма | ккал, по умолчанию 2000 |

**Подключение:** вставь токен → «Подключиться» → выбери базу из списка → таблицы
«Лог питания» и «Сохранённые блюда» определяются по имени автоматически (можно
переопределить в выпадашке). Если у токена нет права `schema.bases:read` —
работает ручной ввод ID (`app...` / `tbl...`).

> OAuth-кнопки «Войти через Airtable» нет намеренно: token-эндпоинт Airtable
> не отдаёт CORS, поэтому браузерный обмен кода невозможен без бэкенда. PAT +
> Metadata API даёт тот же результат («вошёл → выбрал базу») без сервера.
